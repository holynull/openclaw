...
description: Multi-chain cryptocurrency wallet custody operations (EVM + Bitcoin) with SoftHSM hardware security
metadata:
  {
    "openclaw":
      {
        ...
        "requires":
          {
            "env":
              ["SOFTHSM_SERVICE_URL", "SOFTHSM_API_KEY", "SOFTHSM_API_SECRET"],
          },
      },
  }
---
...
Fiduciary cryptocurrency wallet custody for multi-chain operations. Private keys are securely managed by SoftHSM Service, providing hardware-backed security for user funds across 10 accounts (indices 0-9).
...
- All tools support `accountIndex` parameter (0-9) for accessing different wallet accounts
- Default is account 0 if not specified
- Each account has its own independent address and balance

### Balance & Address

- `eth_get_address` - Get wallet address
  - Optional: `accountIndex` (0-9, default: 0)
...
- `accountIndex` (optional): 0-9, defaults to 0
...
- ✅ Use accountIndex (0-9) when accessing non-default accounts

## Security Guidelines

**Fiduciary responsibility:**

- User trusts you with wallet operations secured by SoftHSM Service
- Private keys are hardware-backed and never exposed in plaintext
- 10 accounts (indices 0-9) are available per wallet instance
- All transfers are IRREVERSIBLE after broadcast
- Double-check addresses and amounts
- NEVER skip confirmation protocol
- When in doubt, ask for clarification

**SoftHSM Service architecture:**

- Private keys stored in hardware security module (HSM)
- API authentication via HMAC-SHA256 signed requests
- Supports multiple accounts per BIP44 derivation path
- Public keys cached in HSM for performance
- No mnemonic exposure after initial HSM initialization
...
- ❌ Using accountIndex outside 0-9 range (only 10 accounts initialized)
